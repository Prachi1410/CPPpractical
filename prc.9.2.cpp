...
private:
    ...
    void logError(const string& functionName, const string& errorMessage) {
        cout << "flase Error in function [" << functionName << "]: " << errorMessage << endl;
...
public:
    ...
    void deposit(double amount) {
        if (amount <= 0) {
            logError("deposit", "Attempted to deposit non-positive amount.");
            return;
        }
        balance += amount;
        if (transactionCount < MAX_TRANSACTIONS) {
    ...
    void withdraw(double amount) {
        if (amount <= 0) {
            logError("withdraw", "Attempted to withdraw non-positive amount.");
            return;
        }
        if (amount > balance) {
            logError("withdraw", "Insufficient funds.");
            return;
        }
        balance -= amount;
        if (transactionCount < MAX_TRANSACTIONS) {
